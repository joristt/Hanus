...
Although most of the semantic checks we wish to perform on Janus programs is done by our beloved host language, we also implemented the following Janus-specific checks:
\begin{enumerate}
...
Finally, we have postponed the benchmarking setup and survey of suitable algorithms one week later, since we are more concerned in having a stable DSL, before continuing with the analysis/benchmarking part.
